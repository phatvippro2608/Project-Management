...
@section('contents')
    ...
    <div class="d-flex ms-auto">
        ...
        <a href="{{route('budget.edit', $id) }}" class="btn btn-primary btn-sm">Edit</a>
    </div>
</div>
            ...
            <tbody>
                ...
                @endphp
                    <tr class="table-warning">
                        ...
                        <td>
                            <button type="button" class="btn btn-info btn-sm w-auto rename-btn" data-id="{{$costGroup->project_cost_group_id}}" data-name="{{$costGroup->project_cost_group_name}}" data-bs-toggle="modal" data-bs-target="#renameModal">Rename</button>
                            <button type="button" class="btn btn-danger btn-sm delete-group-btn" data-id="{{$costGroup->project_cost_group_id}}" data-name="{{$costGroup->project_cost_group_name}}">X</button>
                        </td>
                    </tr>
                    @foreach($dataCost as $cost)
                        @if($cost->project_id == $id && $cost->project_cost_group_id == $costGroup->project_cost_group_id)
                            ...
                            <td>{{$cost->project_cost_labor_qty}}</td>
                                ...
                                <td class="text-center align-middle"><button type="button" class="btn btn-danger btn-sm delete-btn" data-id="{{$cost->project_cost_id}}">X</button></td>
                            </tr>
                        @endif
                    @endforeach
                    @php
    ...
    <!-- Choose Group Modal -->
<div class="modal fade" id="chooseGroupModal" tabindex="-1" role="dialog" aria-labelledby="chooseGroupModalLabel" aria-hidden="true">
                ...
                <div class="modal-body">
                    <div class="form-group">
                        ...
                        <select class="form-control" id="existingGroup" name="existingGroup">
                            ...
                            @foreach($dataCostGroup as $costGroup)
                                <option value="{{ $costGroup->project_cost_group_id }}">{{ $costGroup->project_cost_group_name }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="form-group mt-3">
                        <button type="button" class="btn btn-secondary" data-bs-toggle="modal" data-bs-target="#addNewGroupModal">Or Add New Group</button>
                    </div>
                </div>
...
<div class="modal fade" id="addNewGroupModal" tabindex="-1" role="dialog" aria-labelledby="addNewGroupModalLabel" aria-hidden="true">
    ...
    document.addEventListener("DOMContentLoaded", function() {
        ...
        $('.delete-btn').on('click', function() {
            ...
            var projectId = '{{ $id }}';
            var url = '{{ route('budget.delete', ['project_id' => '__PROJECT_ID__', 'cost_id' => '__COST_ID__']) }}'
    ...
    $('.delete-group-btn').on('click', function() {
        var costGroupId = $(this).data('id');
        var costGroupName = $(this).data('name');
        var projectId = '{{ $id }}'; // Assuming you have project id available

        if (confirm('Are you sure you want to delete the cost group "' + costGroupName + '"?')) {
            var url = '{{ route('budget.deleteCostGroup', ['project_id' => '__PROJECT_ID__', 'cost_group_id' => '__GROUP_ID__']) }}'
                .replace('__PROJECT_ID__', projectId)
    ...
    $('#existingGroup').on('change', function() {
        ...
        var url = '{{ route('budget.getCostGroupDetails', ['id' => $id, 'group_id' => '__GROUP_ID__']) }}'
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        $('#costGroupDetails').html('<p class="text-danger">Failed to load details.</p>');
                    }
                },
                error: function(xhr) {
                    $('#costGroupDetails').html('<p class="text-danger">Error: ' + xhr.status + ' ' + xhr.statusText + '</p>');
                }
            });
        } else {
